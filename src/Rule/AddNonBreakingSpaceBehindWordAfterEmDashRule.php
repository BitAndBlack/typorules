...
/**
 * @see \BitAndBlack\TypoRules\Tests\Rules\AddNonBreakingSpaceBehindWordAfterEmDashRuleTest
 */
#[Description(
...
class AddNonBreakingSpaceBehindWordAfterEmDashRule extends AbstractRule implements RuleInterface
{
    ...
    {
        return '/(?<=' . CharactersEnum::MDASH->value . '\s)(\w{1,' . $this->wordMaxLength . '})\s(\w{0,' . $this->wordAheadMaxLength . '})(?!\w)/u';
    }
    ...
    #[Configuration('Configure the minimum length for the word after the em dash. It needs to have a length of at least `3` characters per default.')]
    ...
    #[Configuration('Configure the maximum length for the **second** word after the em dash. By default, it must not have more than `5` characters.')]
